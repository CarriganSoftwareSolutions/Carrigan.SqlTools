...
It provides a wrapper around **ADO.NET** that makes it easier to run the queries generated by Carrigan.SqlTools.Generators.SqlServer. It also includes support for working with the `SqlInvoker`, which can take a `Dictionary<string, object?>` representing a SQL row and create an object instance with those values.  
...
- **Invoker integration**  
  Create object instances from `Dictionary<string, object?>` rows using the `SqlInvoker` class from **Carrigan.SqlTools.Generators.SqlServer**.  
...
Carrigan.SqlTools.SqlServer.Clients.SqlServer is available as a NuGet package:  

```powershell
dotnet add package Carrigan.SqlTools.SqlServer.Clients.SqlServer
```
