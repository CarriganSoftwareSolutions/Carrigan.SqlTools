...
public class SelectBuilderSubqueryTests
{
    ...
    public void Select_WithSubquerySource_RendersSubqueryAsFromSource()
    {
        ...
        Subquery<Customer> subquery = customerGenerator.InternalSubquery(null, null, null, subqueryPredicate, null, null, null, null);
        ...
        SqlQuery query = customerGenerator.InternalSelect(null, subquery, selects, null, outerPredicate, null, null, null, null);

        Assert.Equal("SELECT \"Customer\".\"Id\", \"Customer\".\"Email\" FROM (SELECT \"Customer\".* FROM \"Customer\" WHERE (\"Customer\".\"Name\" = $1)) AS \"Customer\" WHERE (\"Customer\".\"Email\" = $2)", query.QueryText);
    ...
    {
        Subquery<Customer> subquery = customerGenerator.InternalSubquery
        (
            true,
        ...
        Assert.Equal("SELECT DISTINCT \"Customer\".\"Id\", \"Customer\".\"Name\" FROM (SELECT DISTINCT \"Customer\".\"Id\", \"Customer\".\"Name\" FROM \"Customer\" ORDER BY \"Customer\".\"Name\" ASC, \"Customer\".\"Id\" ASC LIMIT 25 OFFSET 50) AS \"Customer\" ORDER BY \"Customer\".\"Name\" ASC, \"Customer\".\"Id\" ASC LIMIT 10", query.QueryText);
        Assert.Equal(CommandType.Text, query.CommandType);
    ...
    public void SelectBuilder_WithSubquerySourceAndJoin_RendersJoinedSql()
    {
        ...
        Assert.Equal("SELECT \"Customer\".\"Id\" AS \"CustomerId\", \"Order\".\"Total\" FROM (SELECT \"Customer\".* FROM \"Customer\" WHERE (\"Customer\".\"Name\" = $1)) AS \"Customer\" INNER JOIN \"Order\" ON (\"Customer\".\"Id\" = \"Order\".\"CustomerId\") WHERE (\"Order\".\"Total\" > $2)", query.QueryText);
        Assert.Equal(CommandType.Text, query.CommandType);
        SqlQueryTestHelper.AssertParameterCount(query, 2);
        SqlQueryTestHelper.AssertParameterValue(query, "$1", "Hank");
        SqlQueryTestHelper.AssertParameterValue(query, "$2", 500m);
    }
}
