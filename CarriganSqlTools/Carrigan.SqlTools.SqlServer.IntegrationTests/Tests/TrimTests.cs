...
public sealed class TrimTests : IClassFixture<LeftRightFixture>
{
    ...
    // SQL Server only supports two-argument LTRIM/RTRIM on SQL Server 2022+ at compatibility level 160+.
    private async Task RequireCharacterTrimSupportAsync()
    {
        await using SqlConnection connection = new(_fixture.UnitTestConnectionString);
        ...
        int compatibilityLevel = Convert.ToInt32(await command.ExecuteScalarAsync());

        if (compatibilityLevel < 160)
            Assert.Skip("The characters argument to LTRIM/RTRIM requires SQL Server 2022 and compatibility level 160 or higher.");
    }
    ...
    [Fact]
    public async Task LRTrim_RemovesSpacesFromBothEnds()
    {
        SqlExpression expression = new LRTrim(CreatePaddedWord("  ", "  "));
        IEnumerable<Words> records = await ExecuteAsync(expression);
    ...
    {
        await RequireCharacterTrimSupportAsync();
