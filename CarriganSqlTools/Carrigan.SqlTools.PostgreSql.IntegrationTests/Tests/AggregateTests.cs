...
public class AggregateTests : IClassFixture<AggregateFixture>
{
    ...
    [Fact]
    public async Task AggregateTest_WithCasts()
    {
        await _fixture.ResetAsync();
        ...
        (
            ...
            new SelectTag(new Cast(new Avg(new Column<Customer>(nameof(Customer.Age))), PostgreSqlTypesProvider.AsDoublePrecision(false)), "Average"),

            new SelectTag(new Cast(new Max(new Column<Customer>(nameof(Customer.Age))), PostgreSqlTypesProvider.AsDoublePrecision(false)), "Max"),

            new SelectTag(new Cast(new Min(new Column<Customer>(nameof(Customer.Age))), PostgreSqlTypesProvider.AsDoublePrecision(false)), "Min"),

            new SelectTag(new Cast(new Count(new Column<Address>(nameof(Address.State))), PostgreSqlTypesProvider.AsDoublePrecision(false)), "Count"),

            new SelectTag(new Cast(new Sum(new Column<Address>(nameof(Address.StreetNumber))), PostgreSqlTypesProvider.AsDoublePrecision(false)), "Sum")
        );
