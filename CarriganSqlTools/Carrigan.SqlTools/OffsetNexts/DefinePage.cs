...
/// </summary>
/// <remarks>
/// This class defines paging behavior (page number and page size).
/// When a <see cref="DefinePage"/> instance is passed to the SQL generator, the generator appends an additional
/// <c>ORDER BY</c> criterion for key columns (if not already present) to ensure stable paging results.
/// </remarks>
/// <example>
...
public class DefinePage : OffsetNext
{
    ...
    /// Thrown when <paramref name="pageNumber"/> is <c>0</c>.
    /// </exception>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="pageSize"/> is <c>0</c>.
    /// </exception>
    public DefinePage(uint pageNumber, uint pageSize)
    {
        if (pageNumber == 0)
        {
            throw new ArgumentOutOfRangeException(nameof(pageNumber), pageNumber, "pageNumber must be greater than zero.");
        }
