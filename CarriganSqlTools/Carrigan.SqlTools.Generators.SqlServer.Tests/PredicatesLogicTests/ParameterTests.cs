...
public class ParameterTests
{
    ...
    public void Parameter_Multiple_Same_Name()
    {
        ...
        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_1").Single().Value!;
        expectedInt = 0;
        Assert.Equal(expectedInt, actualInt);

        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_2").Single().Value!;
        expectedInt = 1;
        Assert.Equal(expectedInt, actualInt);

        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_3").Single().Value!;
        expectedInt = 2;
        Assert.Equal(expectedInt, actualInt);

        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_4").Single().Value!;
        expectedInt = 3;
        Assert.Equal(expectedInt, actualInt);

        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@NotTest_5").Single().Value!;
        expectedInt = 4;
        Assert.Equal(expectedInt, actualInt);

        actualInt = (int)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_6").Single().Value!;
        expectedInt = 10;
        Assert.Equal(expectedInt, actualInt);
    }
    ...
    public void Parameter_Multiple_Same_Name_Complex()
    {
        ...
        char actualChar = (char)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_2").Single().Value!;
        char expectedChar = 'A';
        ...
        actualChar = (char)predicate.ToSqlFragments(Dialect).GetSqlFragmentParameters(Dialect).Where(parameter => parameter.ParameterTag.ToString() == "@Test_4").Single().Value!;
        expectedChar = 'B';
        Assert.Equal(actualChar, expectedChar);
    }
    ...
    public void Constructor_ExplicitSqlType_NullValue()
    {
        ...
        Assert.Equal(new ParameterTag(parameterName), parameter.Name);
